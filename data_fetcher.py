...
class JokerDataFetcher:
    def __init__(self, data_file='joker_results.csv'):
        ...
        self.page_size = 50
        self.max_draw_id = 2902  # Current draw ID as of April 2025
    ...
    def _get_last_draw_id(self):
        ...
        try:
            # Try to get recent draws using the search API
            current_date = datetime.now()
            start_date = current_date - timedelta(days=7)  # Get last week's draws
            
            # Create search payload
            payload = {
                "sort": [{"property": "drawTime", "direction": "DESC"}],
                "page": {"number": 0, "size": 10},
                "filter": {
                    "drawTime": {
                        "from": int(start_date.timestamp() * 1000),
                        "to": int(current_date.timestamp() * 1000)
                    }
                }
            }
            
            print(f"Fetching recent draws from: {self.search_url}")
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, dict) and 'content' in data:
                    draws = data['content']
                    if len(draws) > 0:
                        # Get the highest draw ID
                        max_id = max(draw.get('drawId', 0) for draw in draws)
                        if max_id > 0:
                            return max_id
    ...
    def fetch_historical_data(self, start_year=2000):
        ...
        print(f"Fetching historical data from {start_year}...")
        
        # Load existing data first
        df = self.load_existing_data()
        
        # Start from draw ID 1 (Joker started in November 1997)
        all_draws = []
        current_id = 1
        
        while current_id <= self.max_draw_id:
    ...
    def update_data(self):
        ...
        print(f"Fetching draws from ID {latest_draw_id + 1} to {self.max_draw_id}")
