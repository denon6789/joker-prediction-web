...
class JokerDataFetcher:
    def __init__(self, data_file='joker_results.csv'):
        ...
        self.page_size = 50  # Number of draws per request
    ...
    def _parse_draw(self, draw_data):
        ...
        try:
            ...
            if not winning_numbers or 'list' not in winning_numbers:
                print(f"Invalid draw data format: {json.dumps(draw_data, indent=2)}")
                return None
            ...
            if len(numbers) != 5 or not bonus:
                print(f"Invalid numbers or bonus: numbers={numbers}, bonus={bonus}")
                return None
    ...
    def fetch_historical_data(self, start_year=2000):
        ...
        if all_draws:
            ...
            # Try to get at least recent data
            print("\nTrying to get recent data only...")
            recent_start = current_date - timedelta(days=30)
            draws = self._get_draws_for_date_range(recent_start, current_date)
            if draws:
                for draw in draws:
                    parsed = self._parse_draw(draw)
                    if parsed:
                ...
                if all_draws:
                    ...
                    df = df.sort_values('date')
                    self.save_data(df)
                    print(f"\nGot recent data: {len(df)} draws from {df['date'].min().strftime('%Y-%m-%d')} to {df['date'].max().strftime('%Y-%m-%d')}")
...
if __name__ == "__main__":
    ...
    df = fetcher.fetch_historical_data(start_year=2000)  # This will fetch all historical data
    print(f"Total draws: {len(df)}")
    if len(df) > 0:
        print(f"Date range: {df['date'].min().strftime('%Y-%m-%d')} to {df['date'].max().strftime('%Y-%m-%d')}")
        print("Sample row:")
